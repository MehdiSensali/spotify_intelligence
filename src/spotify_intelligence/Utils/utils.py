...
def get_bearer_token(client_id: str, client_secret: str) -> str:
    if not client_id or not client_secret:
        raise MissingCredentialsError("Client ID and Client Secret must not be null or empty.")
...
if __name__ == "__main__":
    token: str = get_bearer_token(os.getenv("SPOTIFY_CLIENT_ID"), os.getenv("SPOTIFY_CLIENT_SECRET"))
    print(token)